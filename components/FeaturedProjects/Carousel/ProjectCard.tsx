...
export default function ProjectCard({ project, isSelected }: ProjectCardProps) {
  return (
    <>
      {isSelected && (
        <div className="group relative flex h-64 w-full gap-2 overflow-hidden rounded-lg border border-black p-4 transition-all duration-200 ease-in-out hover:border-purple-light hover:shadow-lg dark:border-gray-300/20 dark:bg-gray-300/10 dark:hover:border-purple-dark">
          <Image
            src={project.image}
            alt={project.name}
            width={1280}
          ...
          <div className="flex w-full flex-col items-center justify-between gap-4 pl-3 md:ml-[375px] md:w-1/2">
            <div className="flex flex-col gap-2">
              ...
              <ul className="flex flex-wrap justify-center gap-2">
                {project.tags.map((tag) => (
                  <li
                    key={tag.name}
                    className="rounded-full bg-gray-200 px-2 py-1 text-xs dark:bg-foreground dark:text-black"
                  >
                    {tag.name}
                  </li>
                ))}
